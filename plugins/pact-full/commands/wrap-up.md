...
# /wrap-up v0.1.0 — the end-of-day capture

Close the day. This is the twin of `/standup-prep`: one opens the day, this one
closes it and feeds tomorrow's prep.
...
`/wrap-up` is a back door that closes without it.
...
`[wrap-up {YYYY-MM-DD}] sin movimiento de cliente`. **THAT DATUM IS THE ONE THAT
...
✚ PACT-1088  /standup-prep                   se creó con spec
...
- It does not prepare tomorrow's meeting. That is `/standup-prep`.
