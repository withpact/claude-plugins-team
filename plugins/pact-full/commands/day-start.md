...
# /day-start v0.2.0 — the fixed-output opening block

Open the day as ONE deterministic block. Same command, same board, same block.
The output is **for the engineer, not for the client** — it is never shared in a
meeting.

Many client projects have a daily standup and this is what you run ~15 minutes
before it. Projects without one run it anyway: the moment is opening the day, and
the standup is only its most common occasion.
